...
from app.crud.call import (
    ...
    create_call_event,
    get_call_by_slot_id,
    get_latest_call_event_for_user,
)
...
async def join_call(
    ...
    latest_user_call_event = await get_latest_call_event_for_user(
        db=db,
        call_id=call.id,
        user_id=user.id,
        booking_started_at=latest_slot_event.created_at,
    )

    if (
        latest_user_call_event is None
        or latest_user_call_event.event_type
        != CallEventType.JOINED
    ):
...
async def leave_call(
    ...
    if (
        latest_slot_event is None
        or latest_slot_event.event_type
    ...
    if (
        ...
        raise ForbiddenError(
            "You are not allowed to leave this call"
        )
    ...
    if (
        ...
        raise ConflictError(
            "User is not currently in the call"
        )
